...
function renderEllipse(){
    ...
    var ellipseColor = [];
    for (var i = 0; i <= vertices.length; i++){
        ellipseColor[i] = vec4(1.0, 0.0, 0.0, 1.0);
    }
    ...
    var cBuffer = gl.createBuffer();
    gl.bindBuffer( gl.ARRAY_BUFFER, cBuffer );
    gl.bufferData( gl.ARRAY_BUFFER, flatten(ellipseColor), gl.STATIC_DRAW );
...
function renderSquares() {
    ...
    var black = [];
    for(var i = 0; i <= 3; i++){
        black[i] = vec4(0.0, 0.0, 0.0, 1.0);
    }

    var white = [];
    for(var j = 0; j <= 3; j++){
        white[j] = vec4(1.0, 1.0, 1.0, 1.0);
    }
    var program = initShaders( gl, "vertex-shader", "fragment-shader" );
    gl.useProgram( program );
